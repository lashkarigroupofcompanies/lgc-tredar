...
import './hacker-desk.css';
import { HackerDeskController } from './hacker-desk';
...
if (urlParams.get('settings') === '1') {
  ...
  // Initialize LGC Quantum Hacker Trading Desk
  const desk = new HackerDeskController();
  desk.init();

  const isOldMonitor = urlParams.get('legacy_monitor') === '1';
  if (isOldMonitor) {
